package github.com/hetao29/qrcode/qr
...
func (c *Code) Image(front color.Color, back color.Color) image.Image {
	return &codeImageV2{c, front, back}

}

// codeImage implements image.Image
type codeImageV2 struct {
	*Code
	front color.Color
	back  color.Color
}
...
var (
	whiteColor color.Color = color.Alpha{}
	blackColor color.Color = color.Gray{0x00}
)

func (c *codeImageV2) Bounds() image.Rectangle {
	d := (c.Size + 8) * c.Scale
	return image.Rect(0, 0, d, d)
}

func (c *codeImageV2) At(x, y int) color.Color {
	if c.Black(x, y) {
		return c.front
	}
	return c.back
...
func (c *codeImageV2) ColorModel() color.Model {
